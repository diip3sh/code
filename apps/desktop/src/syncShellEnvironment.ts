...
function applyShellEnvironment(
  env: NodeJS.ProcessEnv,
  platform: NodeJS.Platform,
  shellEnvironment: Partial<Record<string, string>>,
  launchctlPath: string | undefined,
): void {
  ...
  try {
    ...
    const launchctlPath =
      platform === "darwin" && !shellEnvironment.PATH
        ...
        : undefined;
    applyShellEnvironment(env, platform, shellEnvironment, launchctlPath);
  } catch (error) {
...
export async function syncShellEnvironmentAsync(
  ...
  options: {
    platform?: NodeJS.Platform;
    readEnvironment?: AsyncShellEnvironmentReader;
    readLaunchctlPath?: typeof readPathFromLaunchctlAsync;
    userShell?: string;
    logWarning?: (message: string, error?: unknown) => void;
  } = {},
): Promise<void> {
  ...
  const logWarning = options.logWarning ?? logShellEnvironmentWarning;
  const readEnvironment = options.readEnvironment ?? readEnvironmentFromLoginShellAsync;
  const shellEnvironment: Partial<Record<string, string>> = {};

  try {
    for (const shell of listLoginShellCandidates(platform, env.SHELL, options.userShell)) {
      try {
        Object.assign(shellEnvironment, await readEnvironment(shell, LOGIN_SHELL_ENV_NAMES));
        if (shellEnvironment.PATH) {
          break;
        }
      } catch (error) {
    ...
    const launchctlPath =
      platform === "darwin" && !shellEnvironment.PATH
        ? await (options.readLaunchctlPath ?? readPathFromLaunchctlAsync)()
        : undefined;
    applyShellEnvironment(env, platform, shellEnvironment, launchctlPath);
  } catch (error) {
    logWarning("Failed to synchronize the desktop shell environment.", error);
